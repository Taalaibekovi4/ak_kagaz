// src/page/wholesale/WholesaleProduct.jsx
...
// маппинг товара для оптового каталога
const mapProductToCard = (p) => ({
  ...
  // акционные поля — чтобы Cart рисовал "-17% / Акция" и старую цену
  oldPrice:
    p.old_price !== null &&
...
// фильтрация дерева категорий по списку id, которые вообще имеют оптовые товары
const filterCategoryTreeByAllowedIds = (tree, allowedIds) => {
  const dfs = (node) => {
    const children = (node.children || [])
      .map(dfs)
      .filter((child) => child !== null);

    const selfAllowed = allowedIds.includes(node.id);
  ...
  return tree.map(dfs).filter((n) => n !== null);
};

const WholesaleProduct = ({ onAddToCart = () => {} }) => {
  ...
  // ---------- ОПТОВЫЕ ТОВАРЫ ----------
  // отдельный стейт для ids категорий, где вообще есть оптовые товары
  const [wholesaleCategoryIds, setWholesaleCategoryIds] = useState([]);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        const params = { page: currentPage, page_size: 300 };
        ...
        // ОСТАВЛЯЕМ ТОЛЬКО ТОВАРЫ С ОПТОВОЙ ЦЕНОЙ
        results = results.filter((p) => {
          const wp = p.wholesale_price;
          return (
            wp !== null &&
            wp !== undefined &&
            !Number.isNaN(Number(wp)) &&
            Number(wp) > 0
          );
        });

        // сохраняем, какие категории вообще имеют оптовые товары
        ...
        if (categoryFilterMode === "single" && activeCategoryId) {
          results = results.filter((p) => p.category_id === activeCategoryId);
        } else if (categoryFilterMode === "tree" && activeTreeIds.length) {
          results = results.filter((p) =>
  ...
  useEffect(() => {
    ...
    const loadCategories = async () => {
      try {
        ...
        let tree = hasChildrenField ? active : buildCategoryTree(active);

        // если уже знаем, какие категории имеют оптовые товары — режем дерево
        if (wholesaleCategoryIds.length) {
          tree = filterCategoryTreeByAllowedIds(tree, wholesaleCategoryIds);
        }
  ...
  const renderFilters = () => {
    ...
    return (
      <div className="flex flex-col gap-2">
        {/* Все категории (все, где есть опт) */}
        <button
          type="button"
          onClick={() => {
            setActiveCategoryId(null);
      ...
      <div className="max-w-[1200px] mx-auto px-3 sm:px-4 pb-[30px]">
        ...
        <div className="pt-[40px]">
          ...
          <p className="mt-1 text-[14px] text-[#8A8BA0]">
            {count || 0} товаров с оптовыми ценами
          </p>
        </div>
