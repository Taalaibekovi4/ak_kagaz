// src/page/catalog/PopularProducts.jsx
...
// ДОБАВИЛ: promotion / discount / oldPrice / category_id
...
const PopularProducts = ({ onAddToCart = () => {} }) => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        const params = { page: currentPage, promotion: true }; // только акционные (как было)
        ...
        const mapped = results.map(mapProductToCard);

        setProducts(mapped);
        setCount(mapped.length);
        if (mapped.length) setPageSize(mapped.length);
      } catch (e) {
        if (cancelled) return;
        console.error("Failed to load promo products", e);
  ...
  const sortedProducts = useMemo(() => {
    ...
    // "Сначала новые" и "Популярные" оставляем как даёт бэк
    return items;
  }, [sort, products]);
